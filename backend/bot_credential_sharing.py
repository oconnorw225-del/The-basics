...
class BotCredentialSharing:
    ...
    def __init__(self):
        self.config_dir = Path(
            "/home/runner/work/The-basics/The-basics/config")
    ...
    def _load_shared_pool(self) -> Dict[str, Any]:
        ...
        return {"credentials": {}, "discovery_log": [],
    ...
    def _save_shared_pool(self) -> None:
        ...
        self.shared_pool_file.write_text(
            json.dumps(self.shared_pool, indent=2))
    ...
    def _save_requests(self) -> None:
        ...
        self.credential_requests_file.write_text(
            json.dumps(self.credential_requests, indent=2))
    ...
    def bot_discovered_credential(
        ...
        """
        cred_id = f"{credential_type}_{credential_key}".lower().replace(
            " ", "_")
        ...
        self.shared_pool["discovery_log"].append(
            {"bot_id": bot_id, "credential_id": cred_id, "timestamp": datetime.now().isoformat()}
        )
        ...
        for cred_id, cred_data in self.shared_pool["credentials"].items():
            if cred_data["type"] == credential_type:
                if credential_key is None or cred_data["key"] == credential_key:
                    print(
                        f"✅ Fulfilled request from {bot_id} for {credential_type}")
                    return {
                        "success": True,
    ...
    def auto_fill_bot_config(
            self, bot_id: str, required_credentials: List[str]) -> Dict[str, Any]:
        """
        ...
        for cred_type in required_credentials:
            result = self.bot_request_credential(
                bot_id, cred_type, required=True)
        ...
        print(
            f"🤖 Auto-configured {bot_id} with {len(bot_config['credentials'])} credentials")
    ...
    def _fulfill_requests(self, credential_type: str) -> None:
        ...
        for idx, request in enumerate(self.credential_requests):
            if not request["fulfilled"] and request["credential_type"] == credential_type:
                ...
                result = self.bot_request_credential(
                    request["bot_id"], credential_type, request.get("credential_key"))
...
async def main():
    ...
    result2 = credential_sharing.bot_request_credential(
        bot_id="shadowforge_bot", credential_type="api_key"
    )
    print(f"Request result: {result2}")

    # Test auto-fill
    result3 = credential_sharing.auto_fill_bot_config(
        bot_id="new_bot", required_credentials=["api_key", "secret_key"]
    )
