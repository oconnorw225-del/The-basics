...
class SolvencyMonitor:
    ...
    def check_solvency(self, account_data: Dict) -> Dict:
        ...
        status = self._determine_status(
            capital_ratio, liquidity_ratio, leverage)
    ...
    def _determine_status(
        self, capital_ratio: float, liquidity_ratio: float, leverage: float
    ) -> SolvencyStatus:
        ...
        if (
            capital_ratio < self.min_capital_ratio * 0.5
            or liquidity_ratio < self.min_liquidity_ratio * 0.5
            or leverage > self.max_leverage * 1.5
        ):
            return SolvencyStatus.CRITICAL

        if (
            capital_ratio < self.min_capital_ratio
            or liquidity_ratio < self.min_liquidity_ratio
    ...
    def _log_alert(self, assessment: Dict):
        ...
        print(
            f"SOLVENCY ALERT [{assessment['status']}]: {assessment['violations']}")
