...
class DeFiYieldBot:
    ...
    def scan_yield_opportunities(self) -> List[Dict]:
        ...
        filtered = [opp for opp in opportunities if opp["apy"]
                    > 7.0 and opp["risk_score"] < 0.5]
    ...
    def enter_position(self, opportunity: Dict, amount: float) -> Dict:
        ...
        print(
            f"✓ Entered DeFi position: {opportunity['protocol']} {opportunity['asset']}")
    ...
    def exit_position(self, position_id: str) -> Dict:
        ...
        position = next(
            (p for p in self.active_positions if p["position_id"] == position_id),
            None)
...
class ArbitrageBot:
    ...
    def detect_arbitrage(
            self,
            asset: str,
            min_profit_pct: float = 0.3) -> List[Dict]:
        """
        ...
        if opportunities:
            print(
                f"✓ Detected {len(opportunities)} arbitrage opportunities for {asset}")
    ...
    def execute_arbitrage(self, opportunity: Dict, amount: float) -> Dict:
        ...
        print(
            f"  Net Profit: ${net_profit:,.2f} ({execution['profit_pct']:.2f}%)")
