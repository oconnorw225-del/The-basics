...
class BotCoordinator:
        ...
        try:
            ...
            async with self.session.get(url, timeout=timeout) as response:
                if response.status == 200:
                    ...
                    self.bots[bot_name]["health"] = max(0, self.bots[bot_name]["health"] - 10)
                    return False
        except Exception as e:
            logger.error(f"❌ Health check failed for {bot_name}: {e}")
            self.bots[bot_name]["health"] = max(0, self.bots[bot_name]["health"] - 20)
    ...
    async def start_bot(self, bot_name: str) -> bool:
        ...
        if self.kill_switch_active:
            logger.warning(f"⚠️  Cannot start {bot_name}: Kill switch is active")
            return False
    ...
    async def stop_bot(self, bot_name: str, reason: str = "Manual stop") -> bool:
        ...
        try:
            ...
            async with self.session.post(url, json={"action": "stop"}) as response:
                if response.status == 200:
                    ...
                    await self.send_notification("bot_stopped", {"bot_name": bot_name, "reason": reason})
                    return True
                else:
    ...
    async def send_notification(self, alert_type: str, context: Dict):
        ...
        alert_config = self.notification_config["alert_types"].get(alert_type, {})
            ...
            try:
                for bot_name in self.bots.keys():
                    if self.bots[bot_name]["status"] == BotStatus.RUNNING:
                        ...
                        if not healthy:
                            logger.warning(f"⚠️  Health check failed for {bot_name}")
                            await self.send_notification("health_check_failed", {"bot_name": bot_name})
    ...
    async def start_sequential(self):
        ...
        for bot_name in bot_sequence:
            success = await self.start_bot(bot_name)
            if not success:
                logger.error(f"❌ Failed to start {bot_name}, aborting sequence")
                return False
    ...
    async def stop_all(self, reason: str = "Coordinator shutdown"):
        ...
        for bot_name in self.bots.keys():
            if self.bots[bot_name]["status"] in [BotStatus.RUNNING, BotStatus.PAUSED]:
