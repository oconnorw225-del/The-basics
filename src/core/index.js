...
healthMonitor.on('unhealthy', async metrics => {
  await errorHandler.handleError('SYSTEM_UNHEALTHY', new Error('System health check failed'), {
    metrics,
  })
})

healthMonitor.on('memoryLeak', async ({ current, history }) => {
  await errorHandler.handleError('MEMORY_LEAK', new Error('Memory leak detected'), {
    current,
    history,
  })
})
...
function displayStatus() {
  ...
  Object.entries(health.features).forEach(([name, status]) => {
    ...
    console.log(
      `  ${icon} ${healthIcon} ${name.padEnd(15)} - ${status.running ? 'Running' : 'Stopped'}`
    )
  })
...
export { errorHandler, featureManager, healthMonitor, processLinker, shutdownHandler }
