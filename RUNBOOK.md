...
## Railway Deployment and Trading Configuration

### Railway Deployment

The Unified System CI/CD workflow includes optional Railway deployment capabilities:

- **Secret Required**: Railway deployment requires the `RAILWAY_TOKEN` secret to be configured in GitHub repository secrets
- **Automatic Deployment**: When the secret is present, the workflow automatically deploys to Railway on push to the `main` branch
- **Conditional Execution**: If the secret is not configured, the deploy-railway job is safely skipped without failing the workflow

To enable Railway deployment:
1. Obtain a Railway API token from your Railway project settings
2. Add the token as a secret named `RAILWAY_TOKEN` in GitHub repository settings (Settings → Secrets and variables → Actions)
3. The deployment will automatically run on the next push to `main`

### Trading Mode Defaults

**IMPORTANT SAFETY DEFAULTS:**

- **Paper Trading Mode**: All trading operations default to safe/paper mode
- **Live Trading**: Requires explicit enablement on self-hosted runners with appropriate secrets
- **Environment Variable**: Set `TRADING_MODE=paper` (default) or `TRADING_MODE=live` (requires authorization)
- **Production Safety**: Live trading is NEVER enabled on GitHub-hosted runners
- **Self-Hosted Only**: Live trading must be explicitly configured on secure, self-hosted infrastructure with proper access controls

Default configuration in workflows:
```yaml
env:
  TRADING_MODE: paper  # Safe default for all automated deployments
```
